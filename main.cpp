...
namespace
{
  ...
  *********************************************************************************************************************************/
  ...
  void carefully_move_grocery_items( std::stack<GroceryItem> & from, std::stack<GroceryItem> & to )
  {
    ...
    if(!from.empty()){
      std::stack<GroceryItem> temp;
      trace( from, to, temp );
      carefully_move_grocery_items( from.size(), from, to, temp );
    }
    /////////////////////// END-TO-DO (2) ////////////////////////////
  }
}    // namespace
...
int main( int argc, char * argv[] )
{
  ...
  ///////////////////////// TO-DO (5) //////////////////////////////
    ...
    /// arguments.
  std::stack<GroceryItem> myNewCart;
  carefully_move_grocery_items( myCart, myNewCart );
  /////////////////////// END-TO-DO (5) ////////////////////////////
  ...
  {
    groceryItem = worldWideDatabase.find( checkoutCounter.front().upcCode() );
    if( groceryItem != nullptr )
    {
      std::cout << *groceryItem << '\n';
      amountDue += groceryItem->price();
    }
    else
    {
      std::cout << "\"" << checkoutCounter.front().upcCode() << "\""
                ...
                << "not found, your grocery item is free! Bon appetit!" << std::endl;
    }
    checkoutCounter.pop();
  }
  /////////////////////// END-TO-DO (7) ////////////////////////////
