...
GroceryItemDatabase::GroceryItemDatabase( const std::string & filename )
{
  ...
  ///////////////////////// TO-DO (2) //////////////////////////////
    ...
    {
    _groceryItem_database.push_back( std::move(groceryItem) );
    }
...
{
  return find( upcCode, _groceryItem_database.begin() );
}

GroceryItem * GroceryItemDatabase::find( const std::string & upcCode, const std::vector<GroceryItem>::iterator & current_position )
{
  if( current_position == _groceryItem_database.end() ) return nullptr;

  if( upcCode == current_position->upcCode() )
  {
    //return new GroceryItem( *current_position );
    return &(*current_position );
  }
  return find( upcCode, current_position + 1 );
}

std::size_t GroceryItemDatabase::size() const
{
  return _groceryItem_database.size();
}
/////////////////////// END-TO-DO (3) ////////////////////////////
